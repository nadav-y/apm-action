...
- **Registry publish in `mode: release`** (experimental). Set `release-registry-publish: true` to push each packed package to an APM registry via `apm publish` after the matrix-pack step. The GitHub Release (`gh release create`) is still created as normal — this is an additive opt-in, not a replacement.
